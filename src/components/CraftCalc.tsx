...
export function CraftCalc({
  ...
  const breakdown = useMemo(
    () => stashNeeded(recipe, qty, state.materials),
  ...
  return (
    <div className="stack">
      <section className="panel">
        ...
        <p className="muted panel-intro">
          {personal ? (
            ...
            <>
              Pick a craft and quantity — output is <strong>stash needed</strong>.
              Logging deducts those materials from shared stock and adds
              finished products. Restock under <strong>Stock</strong>.
            </>
          )}
        </p>
        ...
        {state.recipes.length === 0 && (
          <p className="empty">
            No craft recipes yet. Paste a screenshot of the next craftable item
            (ingredients / stash) and I&apos;ll add it here. This tab will then
            show how much stash you need for any quantity.
          </p>
        )}
        ...
        {recipe && (
          <>
            ...
            <div className="stash-needed">
              <header className="stash-needed-head">
                <Package size={18} />
                <h4>
                  Stash needed
                  <span className="muted">
              ...
              {breakdown.length === 0 ? (
                <p className="empty">This recipe has no materials listed yet.</p>
              ) : (
                <ul className="stash-needed-list">
                  {breakdown.map((b) => (
                    <li
                      key={b.materialId}
                      className={willDeduct && b.short ? 'short' : ''}
                    >
                      <span className="stash-needed-qty">{b.need}</span>
                      <span className="grow">
                        <strong>{b.name}</strong>
                        <span className="muted">
                          {b.perCraft} each · stock {b.stock}
                          {willDeduct && b.short
                            ? ` · short ${b.short}`
                            : ''}
                        </span>
      ...
      <section className="panel">
        <header className="panel-head">
          <h3>Recipe book</h3>
          <span className="muted">Stash per craft</span>
        </header>
        {state.recipes.length === 0 ? (
          <p className="empty">Waiting on craft screenshots.</p>
        ) : null}
